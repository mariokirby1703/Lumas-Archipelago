...
class CarnivalGamesMiniGolfWorld(World):
    ...
    def generate_early(self):
        start = self.options.starting_world.value - 1
        final = self.options.goal_world.value - 1
        gated = bool(self.options.barker_goal_world_requirement)
        if start < 0:
            start = self.random.choice([i for i in range(9) if not gated or i != final])
        self.starting_world = start
        self.goal_world = None
        if gated:
            if final < 0:
                final = self.random.choice([i for i in range(9) if i != start])
            if final == start:
                raise ValueError("Carnival Games MiniGolf: Goal World must differ from Starting World.")
            self.goal_world = final
        self.counter_mode = gated or bool(self.options.goal.value)
        ...
        unlock_count = 7 if gated else 8
        available = sum(Locations.LOCATION_TABLE[n].world != self.goal_world for n in self.active_locations) if gated else len(self.active_locations)
        if self.required_coins + unlock_count > available:
    ...
    def create_items(self):
        ...
        names.extend([Items.BARKER_COIN] * self.required_coins)
        while len(names) < len(self.active_locations):
    ...
    def create_item(self, name):
        ...
        if name in Items.UNLOCKS or (name == Items.BARKER_COIN and self.counter_mode):
    ...
    def get_filler_item_name(self):
        ...
        if not self.counter_mode and roll < 10:
            return Items.BARKER_COIN
        if (not self.counter_mode and 10 <= roll < 20) or (self.counter_mode and roll < 10):
    ...
    def fill_slot_data(self):
        return {"schema_version": 3, "game": GAME, "seed_name": self.multiworld.seed_name,
